...
def backup_eggroll_data(egg_home: str):
        ...
        subprocess.check_call(['mv', egg_home + '/bin/', egg_home + '/bin_' + datetime.now().strftime("%Y%m%d") + '_bak/'])
         subprocess.check_call(['mv', egg_home + '/conf/', egg_home + '/conf' + datetime.now().strftime("%Y%m%d") + '_bak/'])
...
def cluster_upgrade_sync(src_path: str, dst_path: str, remote_host: str):
    print(f'start upgrade eggroll 2.x.x -> 3.0.x ... src_path={src_path} dst_path={dst_path}')
    subprocess.check_call(
        ["rsync", "-aEvrzhK", "--delete", "--progress", src_path + '/bin/',
         "app@" + remote_host + ":" + dst_path + "/bin/"])
     subprocess.check_call(
         ["rsync", "-aEvrzhK", "--delete", "--progress", src_path + '/conf/',
          "app@" + remote_host + ":" + dst_path + "/conf/"])
    subprocess.check_call(
    ...
    print(f'upgrade eggroll 3.0.x finish')


def upgrade_main(ub_path,nm_file, egg_home, mysql_home, mysql_host, mysql_port, mysql_db, mysql_user, mysql_pwd,
...
def check_file_exits(path):
    if not os.path.exists(path):
        print(f'upgrade eggroll nm_ip_list or mysql_file not exists')
        sys.exit(-1)

def remote_cluster_recover(nm_path,eggroll_home_path: str):
...
def recover_eggroll_data(nm_path,db_home_path, eggroll_home_path: str, host: str, port, database: str, username: str,
    ...
    if os.path.exists(eggroll_home_path + '/conf' + datetime.now().strftime("%Y%m%d") + '_bak/'):
        subprocess.check_call(['mv', eggroll_home_path + '/conf' + datetime.now().strftime("%Y%m%d") + '_bak/', eggroll_home_path + '/conf/'])
    ...
    remote_cluster_recover(nm_path,eggroll_home_path)
...
'''
def restart_server():
     print(f'restart eggroll server ...')
...
def main():
    ...
    print(
        f'input params={args.nm_file, args.egg_home, args.mysql_home, args.mysql_host, args.mysql_port, args.mysql_db, args.mysql_user, args.mysql_pwd,args.mysql_sock, args.mysql_file,args.pkg_base_path,args.recover}')
    ...
    if args.recover == 0:
        ...
        upgrade_main(args.pkg_base_path,args.nm_file, args.egg_home, args.mysql_home, args.mysql_host, args.mysql_port,
                     args.mysql_db, args.mysql_user, args.mysql_pwd,args.mysql_sock, args.mysql_file)
    elif args.recover == 1:
        print(f'recover main ... ...')
        recover_eggroll_data(args.nm_file,args.mysql_home, args.egg_home , args.mysql_host, args.mysql_port, args.mysql_db, args.mysql_user,args.mysql_pwd,args.mysql_sock)
    print(f'end ....')

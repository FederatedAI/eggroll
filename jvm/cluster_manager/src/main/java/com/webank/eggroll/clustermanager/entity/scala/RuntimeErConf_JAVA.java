...
public class RuntimeErConf_JAVA extends ErConf_JAVA {
    ...
    public RuntimeErConf_JAVA(ErSessionMeta_JAVA sessionMeta) {
        ...
        this.getConf().put(SessionConfKeys.CONFKEY_SESSION_ID(), sessionMeta.getId());
        this.getConf().put(SessionConfKeys.CONFKEY_SESSION_NAME(), sessionMeta.getName());
    }
    ...
    public RuntimeErConf_JAVA(StartContainersRequest_JAVA startContainersRequest) {
        ...
        for (Map.Entry<String, String> entry : startContainersRequest.options.entrySet()) {
            this.getConf().put(entry.getKey(), entry.getValue());
        }
        this.getConf().put(SessionConfKeys.CONFKEY_SESSION_ID(), startContainersRequest.sessionId);
        this.getConf().put(SessionConfKeys.CONFKEY_SESSION_NAME(), startContainersRequest.name);
    }

    public RuntimeErConf_JAVA(Map<String, String> conf) {
        super();
        this.getConf().putAll(conf);
    }

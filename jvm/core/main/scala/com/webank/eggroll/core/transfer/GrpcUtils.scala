...
object GrpcClientUtils extends Logging {
  ...
  private def createChannel(endpoint: ErEndpoint,
    ...
    var caCrtPath = CoreConfKeys.CONFKEY_CORE_SECURITY_CA_CRT_PATH.getWith(options)
    if (StringUtils.isBlank(caCrtPath)) {
      caCrtPath = CoreConfKeys.CONFKEY_CORE_SECURITY_CLIENT_CA_CRT_PATH.getWith(options)
    }
    var caCrt: File = null
    if (isSecureChannel) {

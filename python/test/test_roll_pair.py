...
from test.roll_pair_test_assets import (
...
class TestRollPairBase(unittest.TestCase):
    ...
    def test_parallelize_include_key(self):
        rp = self.ctx.parallelize(self.str_generator(True), options=self.store_opts(include_key=True))
    ...
    def test_session_meta(self):
        table = self.ctx.load(
            self.ctx.session_id, "er_session_meta", options={"store_type": StoreTypes.ROLLPAIR_CACHE}
        )

        def get_eggs(data_dir, task):
    ...
    def test_ldl(self):
        ...
        print(222)
        table = self.ctx.load(
            name="fate_flow_detect_table_name",
            namespace="fate_flow_detect_table_namespace",
    ...
    def test_empty_serdes(self):
        rp = self.ctx.load(
            "empty_serdes1", "empty_serdes_ns", options={"create_if_missing": True, "total_partitions": 2}
        )
        rp.put(b"k1", b"v1")
        rp.put(b"k2", b"v2")
    ...
    def test_sc(self):
        ...
        left_rp = self.ctx.load(namespace="ns1", name="testSubtractLeft_10p_8", options=options_left).put_all(
            [
                ("a", 1),
                ("b", 4),
                ("d", 6),
        ...
        right_rp = self.ctx.load(namespace="ns1", name="testSubtractRight_5p_8", options=options_right).put_all(
            [("a", 2), ("c", 4), ("d", 1), ("f", 0), ("g", 1)], options={"include_key": True}
        )
    ...
    def test_cleanup_namespace_specified_store_type(self):
        ...
        self.ctx.cleanup(namespace=namespace, name="*", options={"store_type": StoreTypes.ROLLPAIR_IN_MEMORY})
    ...
    def test_join_self(self):
        ...
        left_rp = self.ctx.load("ns12020", "testJoinLeft2020", options=self.store_opts()).put_all(
            [("a", 1), ("b", 4)], options=self.store_opts(include_key=True)
        )
    ...
    def test_delete(self):
        ...
        table = self.ctx.load("ns1", "test_delete_one", options=self.store_opts(include_key=True)).put_all(
            data, options=self.store_opts(include_key=True)
        )
    ...
    def test_destroy(self):
        ...
        data = [("k1", "v1"), ("k2", "v2"), ("k3", "v3"), ("k4", "v4")]
        table = self.ctx.load("ns12020020618", "test_destroy", options=self.store_opts(include_key=True)).put_all(
            data, options=self.store_opts(include_key=True)
        )
    ...
    def test_take(self):
        ...
        for lst in all_list:
            ...
            self.assertEqual(table.take(n=3, options=options), [item[0] for item in list(table.get_all())[:3]])
    ...
    def test_map_partitions(self):
        ...
        self.assertEqual(
            sorted(table.get_all(), key=lambda x: x[0]),
            [
                ("0_0_0", 0),
                ("0_0_1", 0),
                ("1_1_0", 1),
    ...
    def test_map_partitions_with_reduce(self):
        ...
        rp = self.ctx.load("ns1", "test_map_partitions_with_reduce_3par", options=options).put_all(
        ...
        self.assertEqual(
            sorted(list(table.get_all()), key=lambda x: x[0]),
            [("A_0", 13), ("A_1", 16), ("B_0", 29), ("B_1", 32), ("C_0", 43), ("C_1", 46), ("j_0", 66), ("j_1", 67)],
        )
    ...
    def test_flatMap(self):
        ...
        print(list(res.get_all()))
        print(
            f"get value:{res.get(res.first(options={'keys_only': True}))} of key:{res.first(options={'keys_only': True})}"
        )
    ...
    def test_join(self):
        options = get_default_options()
        left_rp = self.ctx.load("ns1", "testJoinLeft", options=options).put_all(
            [("a", 1), ("b", 4), ("d", 6), ("e", 0)], options={"include_key": True}
        )
        right_rp = self.ctx.load("ns1", "testJoinRight", options=options).put_all(
            [("a", 2), ("c", 4), ("d", 1), ("f", 0), ("g", 1)], options={"include_key": True}
        )
    ...
    def test_join_diff_partitions(self):
        ...
        options_left["total_partitions"] = 10
        options_right["total_partitions"] = 5
        left_rp = self.ctx.load("ns1", "testJoinLeft_10p_6", options=options_left).put_all(
            [
                ("a", 1),
                ("b", 4),
                ("d", 6),
        ...
        right_rp = self.ctx.load("ns1", "testJoinRight_5p_6", options=options_right).put_all(
            [("a", 2), ("c", 4), ("d", 1), ("f", 0), ("g", 1)], options={"include_key": True}
        )
        print(f"left:{get_value(left_rp)}, right:{get_value(right_rp)}")
        ...
        self.assertEqual(
            get_value(left_rp.join(right_rp, lambda v1, v2: v1 + v2)), [("a", 3), ("d", 7), ("f", 3), ("g", 13)]
        )
        self.assertEqual(
            get_value(right_rp.join(left_rp, lambda v1, v2: v1 + v2)), [("a", 3), ("d", 7), ("f", 3), ("g", 13)]
        )

        right_rp = self.ctx.load("ns1", "testJoinRight_10p_7", options=options_right).put_all(
            [
                ("a", 1),
                ("b", 4),
                ("d", 6),
        ...
        left_rp = self.ctx.load("ns1", "testJoinLeft_5p_7", options=options_left).put_all(
            [("a", 2), ("c", 4), ("d", 1), ("f", 0), ("g", 1)], options={"include_key": True}
        )
        print(f"left:{get_value(left_rp)}, right:{get_value(right_rp)}")
    ...
    def test_subtract_by_key(self):
        ...
        options["include_key"] = False
        left_rp = self.ctx.load("namespace20201", "testSubtractByKeyLeft202013", options=options).put_all(
            range(10), options=options
        )
        right_rp = self.ctx.load("namespace2020131", "testSubtractByKeyRight202013", options=options).put_all(
            range(5), options=options
        )
    ...
    def test_subtract_by_key_second(self):
        ...
        left_rp = self.ctx.load("ns1", "testSubtractLeft_10p_3", options=options_left).put_all(
            [("a", 2), ("c", 4), ("d", 1), ("f", 0), ("g", 1)], options={"include_key": True}
        )
        right_rp = self.ctx.load("ns1", "testSubtractRight_5p_3", options=options_right).put_all(
            [("a", 1), ("b", 4), ("d", 6), ("e", 0)], options={"include_key": True}
        )
        print(f"left:{get_value(left_rp)}, right:{get_value(right_rp)}")
        print("111", get_value(left_rp.subtract_by_key(right_rp)))
    ...
    def test_subtract_diff_partitions(self):
        ...
        options_left["total_partitions"] = 10
        options_right["total_partitions"] = 5
        left_rp = self.ctx.load("ns1", "testSubtractLeft_10p_7", options=options_left).put_all(
            [
                ("a", 1),
                ("b", 4),
                ("d", 6),
        ...
        right_rp = self.ctx.load("ns1", "testSubtractRight_5p_7", options=options_right).put_all(
            [("a", 2), ("c", 4), ("d", 1), ("f", 0), ("g", 1)], options={"include_key": True}
        )
        print(f"left:{get_value(left_rp)}, right:{get_value(right_rp)}")
        print("111", get_value(left_rp.subtract_by_key(right_rp)))
        ...
        right_rp = self.ctx.load("ns1", "testSubtractRight_10p_7", options=options_right).put_all(
            [
                ("a", 1),
                ("b", 4),
                ("d", 6),
        ...
        left_rp = self.ctx.load("ns1", "testSubtractLeft_5p_7", options=options_left).put_all(
            [("a", 2), ("c", 4), ("d", 1), ("f", 0), ("g", 1)], options={"include_key": True}
        )
        print(f"left:{get_value(left_rp)}, right:{get_value(right_rp)}")
    ...
    def test_union(self):
        ...
        options["total_partitions"] = 1
        options["include_key"] = False
        left_rp = self.ctx.load("namespace20200110", "testUnionLeft2020", options=options).put_all(
            [1, 2, 3], options=options
        )
        print("left:", left_rp)
        options["include_key"] = True
        right_rp = self.ctx.load("namespace20200110", "testUnionRight2020", options=options).put_all(
            [(1, 1), (2, 2), (3, 3)], options=options
        )
    ...
    def test_union_diff_partitions(self):
        ...
        options_left["total_partitions"] = 10
        options_right["total_partitions"] = 5
        left_rp = self.ctx.load("ns1", "testUniontLeft_10p_6", options=options_left).put_all(
            [
                ("a", 1),
                ("b", 4),
                ("d", 6),
        ...
        right_rp = self.ctx.load("ns1", "testUniontRight_5p_6", options=options_right).put_all(
            [("a", 2), ("c", 4), ("d", 1), ("f", 0), ("g", 1)], options={"include_key": True}
        )
        print(f"left:{get_value(left_rp)}, right:{get_value(right_rp)}")
        ...
        self.assertEqual(
            get_value(left_rp.union(right_rp, lambda v1, v2: v1 + v2)),
            [
                ("a", 3),
                ("b", 4),
                ("c", 4),
        ...
        self.assertEqual(
            get_value(right_rp.union(left_rp, lambda v1, v2: v1 + v2)),
            [
                ("a", 3),
                ("b", 4),
                ("c", 4),
        ...
        right_rp = self.ctx.load("ns1", "testUniontRight_10p_7", options=options_right).put_all(
            [
                ("a", 1),
                ("b", 4),
                ("d", 6),
        ...
        left_rp = self.ctx.load("ns1", "testUniontLeft_5p_7", options=options_left).put_all(
            [("a", 2), ("c", 4), ("d", 1), ("f", 0), ("g", 1)], options={"include_key": True}
        )
        print(f"left:{get_value(left_rp)}, right:{get_value(right_rp)}")

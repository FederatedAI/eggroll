...
@click.option("--timeout-seconds", type=click.INT, required=False, default=60000, help="queue timeout time(s)")
@click.option("--script-path", type=click.Path(exists=True), required=True, help="script path")
...
def submit(ctx, **kwargs):
    ...
    options = {
        "eggroll.container.deepspeed.script.path": os.path.basename(script_path)
    }
...
@click.option("--session-id", type=click.STRING, required=True, help="session id")
@click.option("--content-type", type=click.INT, default=0, required=False, help="ALL:0, MODELS: 1, LOGS: 2")
...
def download(ctx, **kwargs):
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        client.task.download_job_to(session_id=kwargs.get("session_id"), rank_to_path=rank_to_path,
                                    content_type=kwargs.get("content_type"), ranks=kwargs.get("ranks", None))

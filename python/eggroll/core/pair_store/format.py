...
class FileByteBuffer:
    def __init__(self, file):
        ...
        self.file = file
        self.__offset = 0
        self.__size = os.fstat(self.file.fileno()).st_size
    ...
    def write_int32(self, value, offset=None):
        self.__seek_offset(offset)
        self.file.write(pack(">i", value)[0])
...
class PairBinReader(object):
    def __init__(self, pair_buffer):
        ...
        if self.__buf.size() - self.__buf.get_offset() != body_size:
...
class PairBinWriter(object):
    ...
    def write(self, key_bytes, value_bytes):
        ...
        try:
            self.__buf.write_int32(len(key_bytes))
            self.__buf.write_bytes(key_bytes)
            self.__buf.write_int32(len(value_bytes))
            self.__buf.write_bytes(value_bytes)

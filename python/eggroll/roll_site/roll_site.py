...
from eggroll.core.conf_keys import RollSiteConfKeys, CoreConfKeys
...
from eggroll.roll_pair.roll_pair import RollPair, RollPairContext, StoreTypes
from eggroll.roll_pair._batch_stream_status import FINISH_STATUS
    ...
    def __init__(self, roll_site_session_id, rp_ctx: RollPairContext, options: dict = None):
        ...
        if self.push_session_enabled:
            ...
            self._push_session = ErSession(
                session_id=roll_site_session_id + "_push", options=rp_ctx.get_session().get_all_options()
            )
        ...
        self._wait_push_exit_timeout = int(
            RollSiteConfKeys.EGGROLL_ROLLSITE_PUSH_OVERALL_TIMEOUT_SEC.get_with(options)
        )
    ...
    def __init__(self, name: str, tag: str, rs_ctx: RollSiteContext, options: dict = None):
        ...
        if RollSiteBase._receive_executor_pool is None:
            receive_executor_pool_size = int(
                RollSiteConfKeys.EGGROLL_ROLLSITE_RECEIVE_EXECUTOR_POOL_MAX_SIZE.get_with(options)
            )
...
class RollSite(RollSiteBase):
    ...
    def pull(self, parties: list = None):
        ...
        for src_role, src_party_id in parties:
            ...
            futures.append(self._receive_executor_pool.submit(self._impl_instance._pull_one, rs_header))
        return futures
...
class RollSiteGrpc(RollSiteImplBase):
    def __init__(self, name: str, tag: str, rs_ctx: RollSiteContext, options: dict = None):
        ...
        self.push_per_stream_timeout = int(
            RollSiteConfKeys.EGGROLL_ROLLSITE_PUSH_PER_STREAM_TIMEOUT_SEC.get_with(options)
        )
    ...
    def _push_rollpair(self, rp: RollPair, rs_header: ErRollSiteHeader, options: dict = None):
        ...
        def _push_partition(data_dir, ertask: ErTask):
            ...
            with ertask.first_input.get_adapter(data_dir) as db, db.iteritems() as rb:
                ...
                bin_batch_streams = bs_helper._generate_batch_streams(
                    pair_iter=rb, batches_per_stream=batches_per_stream, body_bytes=body_bytes
                )
                ...
                for batch_stream in bin_batch_streams:
                    ...
                    while cur_retry < max_retry_cnt:
                        ...
                        try:
                            stub.push(
                                bs_helper.generate_packet(batch_stream_data, cur_retry), timeout=per_stream_timeout
                            )
                            exception = None
                            break
                        except Exception as e:
    ...
    def _pull_one(self, rs_header: ErRollSiteHeader):
        ...
        try:
            ...
            while wait_time < self.pull_header_timeout and (
                header_response is None or not isinstance(header_response[0], ErRollSiteHeader)
            ):
                header_response = self.ctx.rp_ctx.create_rp(
                    ...
                    partitioner_type=0,
                    options={},
                ).pull_get_header(tag=transfer_tag_prefix + "0", timeout=self.pull_header_timeout)
            ...
            if header_response is None or not isinstance(header_response[0], ErRollSiteHeader):
                raise IOError(
            ...
            for cur_retry in range(self.pull_max_retry):
                ...
                store = self.ctx.rp_ctx.load_rp(
                    name=rp_name, namespace=rp_namespace, store_type=StoreTypes.ROLLPAIR_LMDB
                )
                pull_status, all_finished, total_batches, cur_pairs = store.pull_get_partition_status(
                ...
                if not all_finished:
                    ...
                    clear_future = self._receive_executor_pool.submit(rp.pull_clear_status, transfer_tag_prefix)
                    if data_type == "object":
                        result = b"".join(
                            map(lambda t: t[1], sorted(rp.get_all(), key=lambda x: int.from_bytes(x[0], "big")))
                        )
                        rp.destroy()
                        L.debug(

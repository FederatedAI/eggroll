...
class RollPairIterator(object):
  def __init__(self, store: ErStore, sessionId: str, options={}):
    self.__store = store
    self.__total_partitions = self.__store._store_locator._total_partitions
    self.__command_client = CommandClient()
    self.__data_broker = None
    self.__cur_partition_id = 0
    self.__job = ErJob(id=sessionId,
                       name=RollPair.GET_ALL,
                       inputs=[self.__store],
                       outputs=[self.__store],
                       functors=[],
  ...
  def __connect_next_broker(self):
    if self.__cur_partition_id >= self.__total_partitions:
      return

    def send_command(partition_id):
      task = ErTask(id=f'{self.__job._id}-{partition_id}',
                    name=RollPair.GET_ALL,
      ...
      result = self.__command_client.simple_sync_send(
              input=task,
              output_type=ErTask,
              endpoint=self.__store._partitions[partition_id]._processor._command_endpoint,
              command_uri=CommandURI(f'{RollPair._egg_uri_prefix}/{RollPair.RUNTASK}'),
              serdes_type=SerdesTypes.PROTOBUF)

      return result

    command_thread = Thread(target=send_command, args=[self.__cur_partition_id])
    command_thread.start()

    # check unjoined thread

    transfer_client = TransferClient()
    self.__data_broker = transfer_client.recv(
            endpoint=self.__store._partitions[self.__cur_partition_id]._processor._transfer_endpoint,
            tag=f'{self.__job._id}-{self.__cur_partition_id}')
    self.__cur_partition_id += 1
  ...
  def __next__(self):
    try:
      if self.__data_broker is None:
        self.__connect_next_broker()

      data = next(self.__data_broker)
    except StopIteration as e:
      if self.__data_broker.is_closable() and self.__cur_partition_id < self.__total_partitions:
        self.__connect_next_broker()
        data = next(self.__data_broker)
      else:
        raise StopIteration()

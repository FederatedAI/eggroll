...
class TestStandalone(unittest.TestCase):
    ...
    def test_put_all(self):
        ...
        t = self.ctx.load("ns1", "testPutAll")
    ...
    def test_map_values(self):
        rp = self.ctx.load("ns1", "test_map_values").put_all(range(10), options=get_default_options())
    ...
    def test_collapse_partitions(self):
        rp = self.ctx.load("ns1", "test_collapse_partitions").put_all(range(5), options=get_default_options())
        def f(iterator):
            sum = []
            for k, v in iterator:
                sum.append((k, v))
    ...
    def test_filter(self):
        rp = self.ctx.load("ns1", "test_filter").put_all(range(5), options=get_default_options())
        print(list(rp.filter(lambda k, v: v % 2 != 0).get_all()))

    def test_flatMap(self):
        rp = self.ctx.load("ns1", "test_flat_map").put_all(range(5), options=get_default_options())
        import random
        def foo(k, v):
            result = []
    ...
    def test_glom(self):
        rp = self.ctx.load("ns1", "test_glom").put_all(range(5), options=get_default_options())
    ...
    def test_reduce(self):
        ...
        rp = self.ctx.load("ns1", "testReduce").put_all(range(20), options=get_default_options())
    ...
    def test_sample(self):
        rp = self.ctx.load("ns1", "testSample").put_all(range(100), options=get_default_options())
        print(6 <= rp.sample(0.1, 81).count() <= 14)

    def test_subtract_by_key(self):
        left_rp = self.ctx.load("namespace1206", "testSubtractByKeyLeft1206").put_all(range(10), options=get_default_options())
        right_rp = self.ctx.load("namespace1206", "testSubtractByKeyRight1206").put_all(range(5), options=get_default_options())
        print(list(left_rp.subtract_by_key(right_rp).get_all()))

    def test_union(self):
        options = get_default_options()
        left_rp = self.ctx.load("ns1", "testUnionLeft").put_all([1, 2, 3], options=options)

        options['include_key'] = True
        right_rp = self.ctx.load("ns1", "testUnionRight").put_all([(1, 1), (2, 2), (3, 3)])
        ...
        options['include_key'] = True
        right_rp = self.ctx.load("namespace1206", "testUnionRight1206").put_all([(1, 1), (2, 2), (3, 3)], options=options)

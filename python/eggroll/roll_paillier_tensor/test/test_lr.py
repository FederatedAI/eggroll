...
class TestLR(unittest.TestCase):
  def test_lr(self):
    ...
    data_H = pd.read_csv("/data/czn/data/breast_a_egr_mini.csv").values
    data_G = pd.read_csv("/data/czn/data/breast_b_egr_mini.csv").values
    data_Y = pd.read_csv("/data/czn/data/breast_b_y_egr_mini.csv").values
    ...
    #squre
    fw_G2 = (G).dot(np.ones((10, 1))) ** 2
    fw_H2 = (H).dot(np.ones((20, 1))) ** 2
    ...
    while itr < max_iter:
      ...
      # if itr == 1:
      #   enc_fw_square_G.out("xxxxxxxxxxxx")
      #   #enc_fw_H.out('1231231312312')
      ...
      loss_AA = enc_loss_G.decrypt()

      _loss_A = loss_AA._store.get_all()
      loss_A = _loss_A[0][1][0][0]
